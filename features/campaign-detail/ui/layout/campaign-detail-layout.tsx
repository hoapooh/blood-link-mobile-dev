...
export default function CampaignDetail() {
	...
	const { campaign, isLoading, isError, error, refetch } = useGetCampaignById(id as string);
		...
		return (
			<View className="flex-1 justify-center items-center px-6">
				<Text className="text-red-500 text-center mb-4">
					{error instanceof Error ? error.message : "Không thể tải thông tin chiến dịch"}
				</Text>
	...
	return (
		<Box className="flex flex-col h-full bg-gray-100 w-full">
			<ScrollView className="flex-1 w-full">
				<BannerImage imageUrl={campaign?.banner} title={campaign.name} height={250} />
